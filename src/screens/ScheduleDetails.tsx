...
const ScheduleDetails: React.FC<ScheduleDetailsProps> = ({ schedule, onBack, onDeleteSchedule }) => {
  ...
  // Debug the notes field to see what we're getting
  console.log('🔍 Schedule notes debug:', {
    notes: schedule.notes,
    notesType: typeof schedule.notes,
    notesLength: schedule.notes?.length,
    trimmedLength: schedule.notes?.trim().length,
    isEmpty: !schedule.notes || schedule.notes.trim().length === 0
  });

  // More robust check for notes - exclude common placeholder values
  ...
  const hasValidNotes = schedule.notes && 
    schedule.notes.trim().length > 0 && 
    schedule.notes.trim() !== 'null' && 
    schedule.notes.trim() !== 'undefined' &&
    schedule.notes.trim() !== 'No notes' &&
    schedule.notes.trim() !== 'N/A' &&
    schedule.notes.trim() !== '-' &&
    // Exclude auto-generated placeholder text
    !isAutoGeneratedText(schedule.notes);

  console.log('📝 Has valid notes:', hasValidNotes);
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={[]}>
      ...
      <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.scrollContent}>
        ...
        <ListItem
          title="Locations"
          chips={[`${schedule.venue_name} - ${schedule.city}`]}
          chipBackgrounds={['rgba(0, 0, 0, 0.07)']}
